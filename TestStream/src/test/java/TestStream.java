...
public class TestStream {
    ...
    public void shouldCountSumOfNumbersInTheList() {
        ...
        // when
        Integer quantity = listOfNumbers.stream().mapToInt(x -> x).sum();
    ...
    public void shouldMultiplyAndTransformIntoStringEachElement() {
        ...
        // when
        List<String> multipliedNumbersAsString = numbers.stream().map(x -> String.valueOf(x * 2)).collect(Collectors.toList());
    ...
    public void shouldCheckIfThereIsANumberGreaterThan4() {
        ...
        // when
        boolean anyNumberGreaterThan4 = numbers.stream().anyMatch(x -> x > 4);

        // then
        Assertions.assertThat(anyNumberGreaterThan4).isTrue();
    }
    ...
    public void shouldReduceOnlyPositiveValuesToCalculateMultiplyThem() {
        ...
        // when
        Optional <BigDecimal> reduced = numbers .stream().filter(x -> x.compareTo(BigDecimal.ZERO) > 0).reduce(BigDecimal::multiply);
    ...
    public void shouldGroupBlogPostsByTheirTypes() {
        ...
        // when
        Map <BlogPostType, List<BlogPost>> postsPerType = (Map<BlogPostType, List<BlogPost>>) posts.stream().
